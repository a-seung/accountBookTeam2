<template>
  <div class="container">
    ...
    <div class="transactions">
      <div
        v-for="(groupedTransaction, date) in filteredGroupedTransactions"
        :key="date"
      >
        <div class="transaction-date">
          <span>
            <button
              class="bold-date"
              @click="toggleTransactionVisibility(date)"
            >
        ...
        <div v-show="isVisible(date)">
          <div v-if="groupedTransaction.length > 0">
            <div
              v-for="transaction in groupedTransaction"
              :key="transaction.id"
            >
...
function updatePage() {
  groupedTransactions.value = filteredTransactions.value.reduce(
    (groups, transaction) => {
...
const filteredGroupedTransactions = computed(() => {
  ...
  for (const date in groupedTransactions.value) {
    const dailyTransactions = groupedTransactions.value[date].filter(
      (transaction) => {
